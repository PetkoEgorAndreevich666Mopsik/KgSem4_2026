...
void DirectXApp::BuildPSO()
{
    ...
    // ТРИ render target для G-буфера
...
void DirectXApp::BuildWireframePSO()
{
    ...
    wireframePsoDesc.NumRenderTargets = 3;
...
bool DirectXApp::Initialize() {
    ...
    for (auto& p : parsed)
    {
        ...
        {
            CreateTextureFromTGA("../assets/" + p.DiffuseMap, mat.DiffuseTexture1);
        }
        else
        {
        ...
        {
            CreateTextureFromTGA("../assets/" + p.DiffuseMap2, mat.DiffuseTexture2);
        }
        else
        {
            XMFLOAT3 secondColor;
            secondColor.x = 1.0f - p.Kd.x;
        ...
        device->CreateShaderResourceView(mat.DiffuseTexture1.Get(), &srvDesc, hDescriptor);

        // SRV для texture2 (t1)
        hDescriptor = mCbvHeap->GetCPUDescriptorHandleForHeapStart();
        hDescriptor.ptr += (1 + mat.SrvHeapIndex2) * mCbvSrvUavDescriptorSize;
        device->CreateShaderResourceView(mat.DiffuseTexture2.Get(), &srvDesc, hDescriptor);
